	...
	SubShader
	{
		...
		Pass
		{
			...
			half4 frag ( VertexOutput IN  ) : SV_Target
			{
				...
				float eyeDepth218_g1019 = LinearEyeDepth(SHADERGRAPH_SAMPLE_SCENE_DEPTH( ase_screenPosNorm.xy ),_ZBufferParams);
				float4 unityObjectToClipPos224_g1019 = TransformWorldToHClip(TransformObjectToWorld(IN.ase_texcoord4.xyz));
				float4 computeScreenPos225_g1019 = ComputeScreenPos( unityObjectToClipPos224_g1019 );
				half3 WorldPosFromDepth_SRP567_g1019 = ( _WorldSpaceCameraPos - ( eyeDepth218_g1019 * ( ( _WorldSpaceCameraPos - WorldPosition ) / computeScreenPos225_g1019.w ) ) );
				float3 objToView587_g1019 = mul( UNITY_MATRIX_MV, float4( IN.ase_texcoord4.xyz, 1 ) ).xyz;
				float clampDepth572_g1019 = SHADERGRAPH_SAMPLE_SCENE_DEPTH( ase_screenPosNorm.xy );
				float lerpResult577_g1019 = lerp( ( 1.0 - clampDepth572_g1019 ) , clampDepth572_g1019 , saturate( _ProjectionParams.x ));
				float lerpResult579_g1019 = lerp( _ProjectionParams.y , _ProjectionParams.z , lerpResult577_g1019);
				float3 appendResult582_g1019 = (float3(objToView587_g1019.x , objToView587_g1019.y , -lerpResult579_g1019));
				float3 viewToWorld583_g1019 = mul( UNITY_MATRIX_I_V, float4( appendResult582_g1019, 1 ) ).xyz;
				half3 WorldPosFromDepth_SRP_Ortho584_g1019 = viewToWorld583_g1019;
				float3 lerpResult593_g1019 = lerp( WorldPosFromDepth_SRP567_g1019 , WorldPosFromDepth_SRP_Ortho584_g1019 , ( unity_OrthoParams.w + ( _FogCameraMode * 0.0 ) ));
				#if defined(AHF_CAMERAMODE_PERSPECTIVE)
				float3 staticSwitch589_g1019 = WorldPosFromDepth_SRP567_g1019;
				#elif defined(AHF_CAMERAMODE_ORTHOGRAPHIC)
				float3 staticSwitch589_g1019 = WorldPosFromDepth_SRP_Ortho584_g1019;
				#elif defined(AHF_CAMERAMODE_BOTH)
				float3 staticSwitch589_g1019 = lerpResult593_g1019;
				#else
				float3 staticSwitch589_g1019 = WorldPosFromDepth_SRP567_g1019;
				#endif
				half3 WorldPosFromDepth253_g1019 = staticSwitch589_g1019;
				float3 WorldPosition2_g1019 = WorldPosFromDepth253_g1019;
				float temp_output_7_0_g1024 = AHF_FogDistanceStart;
				float temp_output_155_0_g1019 = saturate( ( ( distance( WorldPosition2_g1019 , _WorldSpaceCameraPos ) - temp_output_7_0_g1024 ) / ( AHF_FogDistanceEnd - temp_output_7_0_g1024 ) ) );
				#ifdef AHF_DISABLE_FALLOFF
				float staticSwitch467_g1019 = temp_output_155_0_g1019;
				#else
				float staticSwitch467_g1019 = ( 1.0 - pow( ( 1.0 - abs( temp_output_155_0_g1019 ) ) , AHF_FogDistanceFalloff ) );
				#endif
				half FogDistanceMask12_g1019 = staticSwitch467_g1019;
				float3 lerpResult258_g1019 = lerp( (AHF_FogColorStart).rgb , (AHF_FogColorEnd).rgb , ( ( FogDistanceMask12_g1019 * FogDistanceMask12_g1019 * FogDistanceMask12_g1019 ) * AHF_FogColorDuo ));
				float3 normalizeResult318_g1019 = normalize( ( WorldPosition2_g1019 - _WorldSpaceCameraPos ) );
				float dotResult145_g1019 = dot( normalizeResult318_g1019 , AHF_DirectionalDir );
				float4 ScreenPos3_g1026 = screenPos;
				float2 UV13_g1026 = ( ( (ScreenPos3_g1026).xy / (ScreenPos3_g1026).z ) * (_ScreenParams).xy );
				float3 Magic14_g1026 = float3(0.06711056,0.00583715,52.98292);
				float dotResult16_g1026 = dot( UV13_g1026 , (Magic14_g1026).xy );
				float lerpResult494_g1019 = lerp( 0.0 , frac( ( frac( dotResult16_g1026 ) * (Magic14_g1026).z ) ) , ( AHF_JitterIntensity * 0.1 ));
				half Jitter502_g1019 = lerpResult494_g1019;
				float temp_output_140_0_g1019 = ( saturate( (( dotResult145_g1019 + Jitter502_g1019 )*0.5 + 0.5) ) * AHF_DirectionalIntensity );
				#ifdef AHF_DISABLE_FALLOFF
				float staticSwitch470_g1019 = temp_output_140_0_g1019;
				#else
				float staticSwitch470_g1019 = pow( abs( temp_output_140_0_g1019 ) , AHF_DirectionalFalloff );
				#endif
				float DirectionalMask30_g1019 = staticSwitch470_g1019;
				float3 lerpResult40_g1019 = lerp( lerpResult258_g1019 , (AHF_DirectionalColor).rgb , DirectionalMask30_g1019);
				#ifdef AHF_DISABLE_DIRECTIONAL
				float3 staticSwitch442_g1019 = lerpResult258_g1019;
				#else
				float3 staticSwitch442_g1019 = lerpResult40_g1019;
				#endif
				half3 Input_Color6_g1020 = staticSwitch442_g1019;
				#ifdef UNITY_COLORSPACE_GAMMA
				float3 staticSwitch1_g1020 = Input_Color6_g1020;
				#else
				float3 staticSwitch1_g1020 = ( Input_Color6_g1020 * ( ( Input_Color6_g1020 * ( ( Input_Color6_g1020 * 0.305306 ) + 0.6821711 ) ) + 0.01252288 ) );
				#endif
				half3 Final_Color462_g1019 = staticSwitch1_g1020;
				half3 AHF_FogAxisOption181_g1019 = AHF_FogAxisOption;
				float3 break159_g1019 = ( WorldPosition2_g1019 * AHF_FogAxisOption181_g1019 );
				float temp_output_7_0_g1021 = AHF_FogHeightEnd;
				float temp_output_167_0_g1019 = saturate( ( ( ( break159_g1019.x + break159_g1019.y + break159_g1019.z ) - temp_output_7_0_g1021 ) / ( AHF_FogHeightStart - temp_output_7_0_g1021 ) ) );
				#ifdef AHF_DISABLE_FALLOFF
				float staticSwitch468_g1019 = temp_output_167_0_g1019;
				#else
				float staticSwitch468_g1019 = pow( abs( temp_output_167_0_g1019 ) , AHF_FogHeightFalloff );
				#endif
				half FogHeightMask16_g1019 = staticSwitch468_g1019;
				float lerpResult328_g1019 = lerp( ( FogDistanceMask12_g1019 * FogHeightMask16_g1019 ) , saturate( ( FogDistanceMask12_g1019 + FogHeightMask16_g1019 ) ) , AHF_FogLayersMode);
				float mulTime204_g1019 = _TimeParameters.x * 2.0;
				float3 temp_output_561_0_g1019 = ( ( ( WorldPosition2_g1019 * ( 1.0 / AHF_NoiseScale ) ) + ( -AHF_NoiseSpeed * mulTime204_g1019 ) ) * 0.05 );
				float temp_output_7_0_g1025 = AHF_NoiseMin;
				float temp_output_7_0_g1023 = AHF_NoiseDistanceEnd;
				half NoiseDistanceMask7_g1019 = saturate( ( ( distance( WorldPosition2_g1019 , _WorldSpaceCameraPos ) - temp_output_7_0_g1023 ) / ( 0.0 - temp_output_7_0_g1023 ) ) );
				float lerpResult198_g1019 = lerp( 1.0 , saturate( ( ( tex3D( AHF_NoiseTexture, temp_output_561_0_g1019 ).r - temp_output_7_0_g1025 ) / ( AHF_NoiseMax - temp_output_7_0_g1025 ) ) ) , ( NoiseDistanceMask7_g1019 * AHF_NoiseIntensity ));
				half NoiseSimplex3D24_g1019 = lerpResult198_g1019;
				#ifdef AHF_DISABLE_NOISE3D
				float staticSwitch42_g1019 = lerpResult328_g1019;
				#else
				float staticSwitch42_g1019 = ( lerpResult328_g1019 * NoiseSimplex3D24_g1019 );
				#endif
				float temp_output_454_0_g1019 = ( staticSwitch42_g1019 * AHF_FogIntensity );
				float3 normalizeResult169_g1019 = normalize( ( WorldPosition2_g1019 - _WorldSpaceCameraPos ) );
				float3 break170_g1019 = ( normalizeResult169_g1019 * AHF_FogAxisOption181_g1019 );
				float temp_output_449_0_g1019 = ( ( break170_g1019.x + break170_g1019.y + break170_g1019.z ) + -AHF_SkyboxFogOffset );
				float temp_output_7_0_g1022 = AHF_SkyboxFogHeight;
				float temp_output_176_0_g1019 = saturate( ( ( abs( temp_output_449_0_g1019 ) - temp_output_7_0_g1022 ) / ( 0.0 - temp_output_7_0_g1022 ) ) );
				float saferPower309_g1019 = abs( temp_output_176_0_g1019 );
				#ifdef AHF_DISABLE_FALLOFF
				float staticSwitch469_g1019 = temp_output_176_0_g1019;
				#else
				float staticSwitch469_g1019 = pow( saferPower309_g1019 , AHF_SkyboxFogFalloff );
				#endif
				float lerpResult179_g1019 = lerp( saturate( ( staticSwitch469_g1019 + ( AHF_SkyboxFogBottom * step( temp_output_449_0_g1019 , 0.0 ) ) ) ) , 1.0 , AHF_SkyboxFogFill);
				half SkyboxFogHeightMask108_g1019 = ( lerpResult179_g1019 * AHF_SkyboxFogIntensity );
				float clampDepth118_g1019 = SHADERGRAPH_SAMPLE_SCENE_DEPTH( ase_screenPosNorm.xy );
				#ifdef UNITY_REVERSED_Z
				float staticSwitch123_g1019 = clampDepth118_g1019;
				#else
				float staticSwitch123_g1019 = ( 1.0 - clampDepth118_g1019 );
				#endif
				half SkyboxFogMask95_g1019 = ( 1.0 - ceil( staticSwitch123_g1019 ) );
				float lerpResult112_g1019 = lerp( temp_output_454_0_g1019 , SkyboxFogHeightMask108_g1019 , SkyboxFogMask95_g1019);
				#ifdef AHF_DISABLE_SKYBOXFOG
				float staticSwitch455_g1019 = temp_output_454_0_g1019;
				#else
				float staticSwitch455_g1019 = lerpResult112_g1019;
				#endif
				half Final_Alpha463_g1019 = staticSwitch455_g1019;
				float4 appendResult114_g1019 = (float4(Final_Color462_g1019 , Final_Alpha463_g1019));
				float4 appendResult457_g1019 = (float4(WorldPosition2_g1019 , 1.0));
				#ifdef AHF_DEBUG_WORLDPOS
				float4 staticSwitch456_g1019 = appendResult457_g1019;
				#else
				float4 staticSwitch456_g1019 = appendResult114_g1019;
				#endif
				
				float3 BakedAlbedo = 0;
				float3 BakedEmission = 0;
				float3 Color = (staticSwitch456_g1019).xyz;
				float Alpha = (staticSwitch456_g1019).w;
...
/*ASEBEGIN
Version=19105
Node;AmplifyShaderEditor.RangedFloatNode;885;-2912,-4864;Half;False;Property;_IsHeightFogShader;_IsHeightFogShader;40;1;[HideInInspector];Create;False;0;0;0;True;0;False;1;1;1;1;0;1;FLOAT;0
Node;AmplifyShaderEditor.RangedFloatNode;879;-3136,-4864;Half;False;Property;_HeightFogGlobal;_HeightFogGlobal;39;1;[HideInInspector];Create;False;0;0;0;True;0;False;1;1;1;1;0;1;FLOAT;0
Node;AmplifyShaderEditor.RangedFloatNode;892;-3328,-4864;Half;False;Property;_Banner;[ Banner ];41;0;Create;True;0;0;0;True;1;StyledBanner(Height Fog Global);False;1;1;1;1;0;1;FLOAT;0
...
Node;AmplifyShaderEditor.FunctionNode;1027;-3328,-4608;Inherit;False;Base;0;;1019;13c50910e5b86de4097e1181ba121e0e;34,360,0,99,1,388,0,557,0,555,0,550,0,354,0,386,0,345,0,351,0,349,0,368,0,378,0,374,0,370,0,450,0,476,0,384,0,339,0,392,0,355,0,116,1,364,0,361,0,500,0,366,0,382,0,376,0,380,0,372,0,343,0,347,0,597,0,603,1;0;3;FLOAT4;113;FLOAT3;86;FLOAT;87
WireConnection;961;2;1027;86
WireConnection;961;3;1027;87
ASEEND*/
//CHKSM=8A0E7EFEED4F81C8F673C35327D1986EB1AB690B